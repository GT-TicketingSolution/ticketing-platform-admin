...
export const addManagerSchema = z
  ...
  .refine(
    (data) =>
      data.attractionManagementEnabled &&
      Array.isArray(data.attractionPermissions) &&
      data.attractionPermissions.length > 0,
    {
      message: "At least one attraction must be assigned to the manager",
      path: ["attractionPermissions"],
    }
  );
