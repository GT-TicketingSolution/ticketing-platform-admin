...
export const seatLayoutSchema = z.object({
  ...
  rows: z.number().int().min(1, "At least 1 row is required").max(MAX_SEAT_ROWS, `Maximum ${MAX_SEAT_ROWS} rows allowed`).optional().default(1),
  cols: z.number().int().min(1, "At least 1 column is required").max(MAX_SEAT_COLS, `Maximum ${MAX_SEAT_COLS} columns allowed`).optional().default(1),
